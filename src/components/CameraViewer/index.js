...
class CameraViewer extends React.Component {
  ...
  computeHeadPoseEstimation(face) {
    const { origin, rotationMatrix } = GeometryUtil.computeHeadPoseEstimation(
      face
    );
    VisUtil.drawAxis(this.drawCanvasCtx, origin, rotationMatrix);
    const { pitch, yaw, roll } = math.rotationMatrixToEulerAngles(
      rotationMatrix
    );
   ...
   * @param value: new coordinates
   */
  updateHandKeypoints = (value) => {
    const { actions } = this.props;
    actions.updateHandeKeypoints(value);
  };
  ...
  makePredictions = async () => {
    try {
      ...
      if ((estimationId = "Face" || "Full Body")) {
      ...
      if ((estimationId = "Hand" || "Full Body")) {
        hands = await this.handModel.estimateHands(inputFrame, flipHorizontal);

        if (hands && hands.length > 0) {
          if (this.cam.isRunning) {
            this.drawCanvasCtx.save();
            this.drawCanvasCtx.translate(0, 0);
            VisUtil.drawHand(this.drawCanvasCtx, hands[0]);
      ...
      if ((estimationId = "Pose" || "Full Body")) {
  ...
  startCamera = async () => {
    ...
    estimationId = this.estimationSelectRef.current.selectedId();
    const canvas = this.videoCanvasRef.current;
