import {TriangulationUtil} from "./triangulation.util"
import * as posenet from '@tensorflow-models/posenet';
import * as handpose from '@tensorflow-models/handpose'
// import * as facemesh from '@tensorflow-models/face-landmarks-detection'
import StreamData from "./stream_data";

const websockettoRoom = 'pythonclient'
const math = window.math;
const fingerLookupIndices = {
    thumb: [0, 1, 2, 3, 4],
    indexFinger: [0, 5, 6, 7, 8],
    middleFinger: [0, 9, 10, 11, 12],
    ringFinger: [0, 13, 14, 15, 16],
    pinky: [0, 17, 18, 19, 20],
};
let drawCanvasCtx
export default class VisUtil {

    static drawHand(video, ctx, hands, num, color, videoHeight, videoWidth) {
        let drawCanvasCtx = ctx
        let landmarks = hands[0]
        drawCanvasCtx.width = videoWidth;
        drawCanvasCtx.height = videoHeight;
        video.width = videoWidth;
        video.height = videoHeight;

        drawCanvasCtx.clearRect(0, 0, videoWidth, videoHeight);
        drawCanvasCtx.strokeStyle = 'red';
        drawCanvasCtx.fillStyle = 'red';

        drawCanvasCtx.translate(drawCanvasCtx.width, 0);
        drawCanvasCtx.scale(-1, 1);

        this.drawKeypoints(landmarks, drawCanvasCtx);
    }


    static drawKeypoints(ctx, perdictions) {
        drawCanvasCtx = ctx
        let landmarks = perdictions[0]
        const keypointsArray = perdictions;

        for (let i = 0; i < keypointsArray.length; i++) {
            const y = keypointsArray[i][0];
            const x = keypointsArray[i][1];
            this.drawPoint(x - 2, y - 2, 3);
        }
        const fingers = Object.keys(fingerLookupIndices);
        for (let i = 0; i < fingers.length; i++) {
            const finger = fingers[i];
            const points = fingerLookupIndices[finger].map(idx => landmarks[idx]);
            this.drawPath(points, false);
        }
    }

    static drawPoint(y, x, r) {
        drawCanvasCtx.beginPath();
        drawCanvasCtx.arc(x, y, r, 0, 2 * Math.PI);
        drawCanvasCtx.fill();
    }

    static drawPath(points, closePath) {
        const region = new Path2D();
        region.moveTo(points[0][0], points[0][1]);
        for (let i = 1; i < points.length; i++) {
            const point = points[i];
            region.lineTo(point[0], point[1]);
        }

        if (closePath) {
            region.closePath();
        }
        drawCanvasCtx.stroke(region);
    }
}


//
// static drawHand(ctx, hand,scale, color,pointSize) {
//     let keypoints  = hand.landmarks
//     this.drawHandKeypoints(ctx,keypoints,color,scale, pointSize)
// }
//
// static drawHandPoint(ctx,y, x, r,color, pointSize) {
//     ctx.beginPath();
//     ctx.arc(x, y, r, 0, 2 * Math.PI);
//     ctx.fillStyle = "blue";
//     ctx.fill();

// }
// static drawHandKeypoints(ctx,keypoints,color,scale, pointSize) {
//
//     let wholeHand  = []
//     const keypointsArray = keypoints;
//     const fingers = Object.keys(fingerLookupIndices);
//     for (let i = 0; i < fingers.length; i++) {
//         let data ={}
//         const finger = fingers[i];
//         const points = fingerLookupIndices[finger].map(idx => keypoints[idx]);
//         //Send Data to be streamed
//         data.points = points
//         // data.finger = finger
//         wholeHand.push(data)
//         // console.log(data)
//         // StreamData.send_data(JSON.stringify(wholeHand),websockettoRoom)
//         this.drawHandPath(ctx,points, false,color);
//     }
//     StreamData.send_data(JSON.stringify(wholeHand),websockettoRoom)
//     for (let i = 0; i < keypointsArray.length; i++) {
//         const y = keypointsArray[i][0];
//         const x = keypointsArray[i][1];
//         this.drawHandPoint(ctx, x * scale, y * scale, 2, color, pointSize);
//         //this.drawHandPoint(ctx,x - 2, y - 2, 3,color);
//     }
// }

//   static drawHandPath(ctx, points, closePath,color) {
//     const region = new Path2D();
//     region.moveTo(points[0][0], points[0][1]);
//     for (let i = 1; i < points.length; i++) {
//         const point = points[i];
//         region.lineTo(point[0], point[1]);
//     }
//
//     if (closePath) {
//         region.closePath();
//     }
//     ctx.strokeStyle = color;
//     ctx.stroke(region);
// }

